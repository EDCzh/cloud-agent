...
#注释：当消息数量超过这个阈值时，触发修剪（trim）操作
#简单来说：当对话历史中的消息超过 10 条时，系统会自动压缩或清理旧消息。
...
class ShortTermMemory:
    ...
    async def initialize(self) -> None:
        ...
        try:
            #这是导入 Redis 的异步 Python 客户端库：
            #redis.asyncio：Redis 官方提供的异步版本客户端模块
            #as aioredis：给它起一个别名 aioredis，方便后续使用
    ...
    async def save_messages(
        ...
        try:
            ...
            await self._client.set(
                self._key(user_id, session_id),#调用私有方法生成唯一的键
                json.dumps(messages, ensure_ascii=False),#ensure_ascii=False保留原始中文
                ex=self._ttl,#设置过期时间
            )
            logger.debug(
    ...
    #保留所有 system 消息 + 最近的 6 条非 system 消息
    @staticmethod
