...
def extract_knowledge_graph(file_path: str) -> dict:
    ...
    model_name = os.getenv("MODEL", "qwen-turbo")
    ...
    prompt = ChatPromptTemplate.from_messages([
        ...
        2. **关系(Edges)**：识别实体间的约束和关联。
           - 关系类型应简洁且为大写(如 SUPPORTS, CONTAINS, RESTRICTS, REQUIRES, RESOLVED_BY, BELONGS_TO 等)。
    ...
    text_splitter = RecursiveCharacterTextSplitter(
        chunk_size=800,  # 减小分块大小，提高处理速度
        chunk_overlap=50,  # 适当减小重叠
        separators=["\n## ", "\n### ", "\n\n", "\n", " ", ""]
    )
    ...
    for i, chunk in enumerate(chunks):
        ...
        try:
            kg_result = chain.invoke({"text": chunk}, config={"max_concurrency": 1})
    ...
    print(
        f"✅ 所有片段提取并合并完成！共提取 {len(final_kg['nodes'])} 个节点，{len(final_kg['edges'])} 条关系。JSON 已保存至: {output_json}")
    return final_kg
