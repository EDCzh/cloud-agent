...
"""
_extract_keywords 是一个关键词提取工具函数。
它的任务是从用户的一句自然语言提问中，剥离出最核心、最有检索价值的“词根”，供后续的图谱搜索或向量匹配使用。
"""
...
"""
_fallback_graph_keyword_search 是一个降级兜底函数。
它的核心作用是：当复杂的“自然语言转 Cypher 查询”（Text-to-Cypher）失败，
或者 Neo4j 的 APOC 插件不可用时，它提供一种更简单、更鲁棒的方式，通过关键词匹配直接从知识图谱中检索相关信息。"""
