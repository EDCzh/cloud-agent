...
工作要求：
- **核心原则**：
  - 遇到“是什么”、“怎么做”、“有什么规定/政策/限制”等**文本解释类**问题，**必须首选 `query_vector_db`**。
  - 遇到“某实例有多少网卡”、“A包含B吗”、“某地域有哪些型号”等**实体关系/参数查询**问题，**必须首选 `query_knowledge_graph`**。

- **示例参考**：
  - 用户问：“退款有什么限制？” -> 这是一个政策解释问题 -> **调用 `query_vector_db`**。
  - 用户问：“g8a.xlarge 有几块网卡？” -> 这是一个具体参数问题 -> **调用 `query_knowledge_graph`**。

- 只有在向量检索无法提供足够信息，且问题明显涉及复杂实体关联时，才考虑补充调用图谱工具。
- 严禁对纯文本政策类问题直接调用图谱，除非向量检索明确失败。
...
"""
        ...
        #这几行代码是整个 ProductAgent 的**“大脑组装车间”**
